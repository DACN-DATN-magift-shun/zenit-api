...
{
    public class UpdateWalletEventHandler(MoneyTransferService moneyTransferService) : EventUpdateHandler<Wallet>
    {
        public override async Task Handle(Wallet data, List<AuditDataChange>? dataChanges)
        {
            await moneyTransferService.HandleUpdateWalletAsync(data, dataChanges);
        }
    }
}